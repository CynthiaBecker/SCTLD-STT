---
title: "SCTLD_STT - Code to Reproduce Figures in Manuscript"
...
setwd("~/Documents/Apprill_lab/SCTLD_USVI_GitHub/SCTLD-STT")
```

## All R Figures reproduced here

```{r Install all necessary packages}
...
write.table(DNAextraction.contam, "DNAextraction_contaminants_decontam.txt", sep = "\t", col.names = NA) #Summary table - part of Appendix
...
write.table(SWsyringe.contaminants, "SyringeMethod_contaminants_decontam.txt", sep = "\t", col.names = NA) #This is a summary of the Syringe method contaminants part of appendix
...
## Figure 1 - Sampling locations in St. Thomas, U.S. Virgin Islands
...
shapefile <- readOGR("~/Documents/Apprill_lab/SCTLD_USVI_GitHub/SCTLD-STT/Shapefile_for_Map", "vt021tk4894")
shape_df <- fortify(shapefile) # Create the shapefile you will use in ggplot2

usa <- st_read("~/Documents/Apprill_lab/SCTLD_USVI_GitHub/SCTLD-STT/Shapefile_for_Map", "vt021tk4894")
...
ggplot(data = world) +
  ...
  geom_point(aes(y = 18.342194, x = -64.911193), pch = 21, fill ="red", size = 7) +
  theme(panel.grid.major = element_line(color = gray(.7), linetype = "dashed", size = 0.5), panel.background = element_rect(fill = "aliceblue"))
...
```{r Manuscript analysis PCoA Coral}
#Upload data
...
dis2.meta$conditionshort <-factor(dis.meta$conditionshort, levels=c("HH","HD","DD"))
dis2.meta$healthstate <-factor(dis.meta$healthstate, levels=c("H","D"))
...
```{r Manuscript analysis PCoA Seawater}
#Upload data
#import the ASV data that has been filtered for contaminants and low abundance reads. 
ASV <- read.table("ASV_STT2020_filt_decontam.txt", sep = "\t", row.names = 1, header = TRUE)
taxa <- as.matrix(read.table("taxonomy_STT2020_nospecies_filt_decontam.txt", sep = "\t", row.names = 1, header = TRUE))
...
## Figure 4. Disease-enriched ASVs identified by differential abundance analysis as significantly associated with SCTLD in at least one coral. 
...
```{r Bubble plot of disease-associated ASVs in coral tissue samples}
...
ggplot(long.withtaxa.nonzero, aes(x = sample, y = ASV, fill = Genus)) +
  ...
  labs(title = "Relative Abundance of SCTLD lesion-associated ASVs across all corals", size = "Relative Abundance", color = "Genera") +
...
```{r Bubble plot of potential pathogens in the coral seawater}
...
ggplot(long.withtaxasw.nonzero, aes(x = sample, y = ASV, fill = Genus)) +
  ...
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  labs(title = "Relative Abundance of SCTLD lesion-associated ASVs in SW surrounding coral", size = "Relative Abundance", color = "Genera") +
  guides(fill = guide_legend(override.aes = list(size = 4)))
...
```{r Manuscript Analysis PCoA All}
#Upload data
#import the ASV data that has been filtered for contaminants and low abundance reads. 
ASV <- read.table("ASV_STT2020_filt_decontam.txt", sep = "\t", row.names = 1, header = TRUE)
taxa <- as.matrix(read.table("taxonomy_STT2020_nospecies_filt_decontam.txt", sep = "\t", row.names = 1, header = TRUE))
...
plot_grid(p1,p2,p3,p4, labels = c("A","B","C","D"), ncol = 2, nrow = 3)
#dev.off()
...
## Supplementary Figures 5 - 8. Differential abundance analyses between healthy and diseased coral tissues and graphs showing differential abundant ASVs
```{r Corncob analysis and graphing}
otu <- read.table("ASV_STT2020_filt_decontam.txt",sep="\t",header=TRUE, row.names=1)
taxon <- as.matrix(read.table("taxonomy_STT2020_nospecies_filt_decontam.txt", sep="\t", header=TRUE, row.names=1))
samples <- read.table("metadata_STT2020_filt_decontam.txt",sep="\t",header=T,row.names=1)
...
mcav.daplot <- ggplot(mcav.forgraph, aes(x = asvtaxa, y = Estimate, fill = Family)) +
...
mmea.daplot <- ggplot(mmea.forgraph, aes(x = asvtaxa, y = Estimate, fill = Family)) +
  geom_errorbar(aes(ymin = Estimate-StdError, ymax = Estimate+StdError), color = "black", width = .3, position=position_dodge(.9)) +
  geom_point(size = 4, pch = 21) + 
...
ofra.daplot <- ggplot(ofra.forgraph, aes(x = asvtaxa, y = Estimate, fill = Family)) +
  geom_errorbar(aes(ymin = Estimate-StdError, ymax = Estimate+StdError), color = "black", width = .3, position=position_dodge(.9)) +
  geom_point(size = 4, pch = 21) + 
...
cnat.daplot <- ggplot(cnat.forgraph, aes(x = asvtaxa, y = Estimate, fill = Family)) +
  geom_errorbar(aes(ymin = Estimate-StdError, ymax = Estimate+StdError), color = "black", width = .3, position=position_dodge(.9)) +
  geom_point(size = 4, pch = 21) + 
